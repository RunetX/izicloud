...
Функция ВыполнитьМетодGET(Запрос)
	...
	Иначе
		...
		Если КлючЗапроса = Неопределено Тогда
			...
			Результат = СформироватьСтруктуруОшибки(Ошибка);
		Иначе
			Параметры = Запрос.ПараметрыЗапроса.Получить("parameters");
			Если ЗначениеЗаполнено(Параметры) Тогда
				СоответствиеПараметров = Новый Соответствие;
				СтруктураИсходныхПараметров = ПреобразоватьJSONСтрокуВСтруктуру(Параметры);
				Для каждого ЭлементСтруктурыПараметров Из СтруктураИсходныхПараметров Цикл
					Если ТипЗнч(ЭлементСтруктурыПараметров.Значение) = Тип("Массив") Тогда
						Попытка
							СоответствиеПараметров.Вставить(ЭлементСтруктурыПараметров.Ключ, "'"+СтрСоединить(ЭлементСтруктурыПараметров.Значение, "','")+"'");
						Исключение
							ОбработкаДанных.ЗаписатьВЛог(ОписаниеОшибки());	
						КонецПопытки;
					Иначе
						СоответствиеПараметров.Вставить(ЭлементСтруктурыПараметров.Ключ, ЭлементСтруктурыПараметров.Значение);	
					КонецЕсли;
				КонецЦикла;
			Иначе
				СоответствиеПараметров = Неопределено;
			КонецЕсли;
			АйкоМассивСоответствий = IIKOСправочники.ВыполнитьНаСервереАйко(ПараметрыПодключения.IIKO_DBMS_TYPE, 
																			КлючЗапроса,
