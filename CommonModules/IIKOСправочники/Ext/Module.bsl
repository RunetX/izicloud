...
Функция ВыполнитьНаСервереАйко(ТипБД, ТипЗапроса, СоответствиеПараметров=Неопределено, ПП) Экспорт// ПП - параметры подключения
	...
	Если ЗапросКАйко <> Неопределено Тогда
		...
		Иначе
			Возврат ОбработкаДанных.ЗаписатьВЛог("Сервер HTTP вернул ошибку "+HTTPОтвет.КодСостояния);
			Возврат Неопределено;
		КонецЕсли;
	Иначе
...
Функция ПолучитьПараметрыЗапросаКСерверуАйко(СтруктураОбъекта, П)
	...
	Если СтруктураОбъекта.ИмяМетода = "createProduct" Тогда
		...
		XMLОбъекта = ВернутьТекстXMLНоменклатуры(СтруктураОбъекта, П);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция ВернутьТекстXMLНоменклатуры(СтруктураНоменклатуры, П)
	ИдНоменклатуры = Новый УникальныйИдентификатор;
	Возврат 
	...
	|	<product eid="""+ИдНоменклатуры+""">
	|		<mainUnit>"+		СтруктураНоменклатуры.ЕдиницаИзмерения+"</mainUnit>
	|		<type>"+			СтруктураНоменклатуры.Тип+"</type>
	|		<num>"+				СтруктураНоменклатуры.Артикул+"</num>
	|		<parent>"+			СтруктураНоменклатуры.Родитель+"</parent>
	|		<name>
	|			<customValue>"+	СтруктураНоменклатуры.Наименование+"</customValue>
	|		</name>
	|		<id>"+ИдНоменклатуры+"</id>
	|	</product>
	|	<overrideFastCode>true</overrideFastCode>
	|	<overrideCode>false</overrideCode>
	|</args>";	
КонецФункции
