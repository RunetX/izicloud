...
Функция ПолучитьВерсиюОбъекта(ПП)
	МассивОтвета = IIKOСправочники.ВыполнитьНаСервереАйко(ПП.IIKO_DBMS_TYPE,"НомерОбъекта",,ПП);
...
Функция ПолучитьHTTPЗаголовки(РазмерФайлаОтправки, П)
	АйкоВерсия = IIKOСправочники.ВыполнитьНаСервереАйко(П.IIKO_DBMS_TYPE,"Версия",,П)[0].Получить("serverversion");
